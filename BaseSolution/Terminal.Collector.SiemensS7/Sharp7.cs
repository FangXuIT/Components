...
namespace Terminal.Collector.SiemensS7
{
    ...
    public static class S7
    {
        ...
        public static void SetStringAt(byte[] Buffer, int Pos, int MaxLen, string Value, Encoding encoding)
        {
            ...
            encoding.GetBytes(Value, 0, size, Buffer, Pos + 2);
        }
        #endregion
    ...
    public class S7Client
    {
        ...
        {
            return TCPConnect(_PLCPort);
        }

        private int TCPConnect(int port)
        {
            if (_LastError == 0)
                try
                {
        ...
        public int Connect(int port=102)
        {
            _LastError = 0;
            Time_ms = 0;
            int Elapsed = Environment.TickCount;
            if (!Connected)
            {
                TCPConnect(port); // First stage : TCP Connection
                if (_LastError == 0)
                {
        ...
        public int ConnectTo(string Address,int port, int Rack, int Slot)
        {
            UInt16 RemoteTSAP = (UInt16)((ConnType << 8) + (Rack * 0x20) + Slot);
            SetConnectionParams(Address, 0x0100, RemoteTSAP);
            return Connect();
